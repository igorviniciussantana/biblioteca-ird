...
export default function Livros({ livro }) {
  ...
  const filtraBusca = livro.filter(({ titulo }) =>
    titulo.toLowerCase().includes(busca.toLowerCase())
  );

  const numeroLivros = function () {
    if (livro.length > 1) {
      return <span>{livro.length} livros registrados</span>;
    } else {
      return <span>{livro.length} livro registrado</span>;
    }
  };
  ...
  if (livro.length >= 1) {
      ...
      <>
        ...
        <div className="container">
          ...
          <main className="mainLivros">
            ...
            <input
              type="text"
              className="livrosInput"
              placeholder="Buscar livro..."
              onChange={(ev) => setBusca(ev.target.value)}
            />
            <Link href="/addLivro">
              <button className="livrosButton">Novo Livro</button>
            </Link>
                ...
                return (
                  <div key={livro.id} className="livroCard">
                    <Link href={"livros/" + livro.id}>
                      <h1 className="livroTitulo">{livro.titulo}</h1>
                    </Link>
                    <p>
                      <strong>Data de Publicação:</strong>{" "}
                      {livro.data_publicacao.substr(0, [10])}
                    </p>
                    <p>
                      <strong>ID do Autor:</strong> {livro.autorId}
                    </p>
                    <p>
                    ...
                    <div className="buttonsDiv">
                      <Link href={"autores/" + livro.autorId}>
                        <button type="button" className="buttonAutor">
                          Autor
                        </button>
                      </Link>
                      <button
    ...
    return (
      <>
        <Head>
          <title>Biblioteca IRD - Bem Vindo!</title>
        </Head>
        <div className="container">
          <Menu />
          <div className="brilho"></div>
          <main className="mainLivros">
            ...
            <h1 className="livrosTitle">
              <center>Não há livros</center>
            </h1>
          </main>
        </div>
      </>
