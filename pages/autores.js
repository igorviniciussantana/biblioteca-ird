...
const filtraBusca = autor.filter(({nome, sobrenome}) => nome.toLowerCase().includes(busca.toLowerCase()) + sobrenome.toLowerCase().includes(busca.toLowerCase()))
const numeroAutores = function(){

if(autor.length > 1){
return(

  <span>{autor.length} autores registrados</span>
)
}else{

  return(

    <span>{autor.length} autor registrado</span>
  )
...
<div key={autor.id} className="autorCard">
              <Link href={'autores/'+ autor.id}><h1 className='autorNome'>{autor.nome} <br/>  {autor.sobrenome}</h1></Link>
              <p>Nascimento: {autor.data_nascimento.substr(0,[10])}</p>
              <p>Id do Autor: {autor.id}</p>
              <div className="buttonsDiv">
              <Link href={'autores/livros/' + autor.id}><button type="button" className='buttonLivros'>Livros</button></Link>
...
<input type="text" className='autoresInput' placeholder='Buscar autor...' onChange={(ev) => setBusca(ev.target.value)}/>
<Link href='/addAutor'><button className='autoresButton'>Novo Autor</button></Link>
<h1 className='autoresTitle'><center>Não há autores</center></h1>
</main>
